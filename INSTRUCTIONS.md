...
**Project Submission**
**Requirements**
# Edit the README.md file in this repo with instructions on how to run your site, and commit it along with your code to this repo.
# Ensure that the frontend and backend specifications below are met
# Ensure that there is some minimum level of unit or integration testing

**Bonus**: 
# Signup and deploy your site to a platform such as Heroku.
# The easier it is for a fresh engineer to get your project up and running locally on their own computer, the better
...
Create a single page application that allows a user to purchase the magic potion.

Although we have a soft spot for React and Laravel, you may in the language of your choice. 
...
1. Capture the following user information in a form:
    - First and Last Name
    - Email
        - provide a *simple* way to validate the email, such as having the user type it in twice
    - Address (street1, street2, city, state, zip)
        - for this exercise, ignore international addresses, and assume that all addresses are in the US
    - Phone Number
    ...
    - A Quantity field that allows user to specify how many potions they'd like to purchase.
        - Ensure that users can only order a maximum of 3 magic potions

    - A display field that displays the product of the quantity multiplied by the base unit price of 49.99 per potion.
        - This field should update with a new value when the user changes the quantity of potions
...
2. A button that submits the form and validates all fields are completed properly and that none are empty. All fields should be evaluated as strings/text except for the quantity field, which should be a number/integer.
    ...
    - submit multiple orders under the same name
    - are there other cases we should consider? If any occur to you, you might want to address them in your API or call them out in your README

    - the API should return the correct response codes for the results, so if everything is fine, it should return an `HTTP 200 Success` or `HTTP 204 No Content` code (depending on the content of the response), and if there is something wrong with the input, it should return an `HTTP 400 Bad Request` code.
...
3. A notification to the user after the form was submitted to the API
    ...
    - You have the option to choose how and where data validation is implemented (client side? Server side? Both?).
